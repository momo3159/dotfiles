if vim.g.vscode then
  ...
  keymap.set('n', '<leader>e', "<Cmd>call VSCodeNotify('workbench.action.closeActiveEditor')<CR>", opts)
  keymap.set('n', '<leader>w', "<Cmd>call VSCodeNotify('workbench.action.closeOtherEditors')<CR>", opts)
else
  require'barbar'.setup {
    -- Enable/disable animations
    animation = false,
    -- Enable/disable auto-hiding the tab bar when there is a single buffer
    auto_hide = false, -- 合わせてshowtabline = 1 に設定しておく
